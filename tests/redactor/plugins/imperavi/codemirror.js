	...
	$.Redactor.prototype.codemirror = function()
	{
		return {
			init: function()
			{
				...
				var settings = (typeof this.opts.codemirror !== 'undefined') ? $.parseJSON(this.opts.codemirror) : { lineNumbers: true };
				if(settings.firstLineNumber !== undefined) settings.firstLineNumber = parseInt(settings.firstLineNumber);
				if(settings.indentUnit !== undefined) settings.indentUnit = parseInt(settings.indentUnit);
				if(settings.tabSize !== undefined) settings.tabSize = parseInt(settings.tabSize);
				if(settings.undoDepth !== undefined) settings.undoDepth = parseInt(settings.undoDepth);
				if(settings.firstLineNumber !== undefined) settings.firstLineNumber = parseInt(settings.firstLineNumber);
				settings.autoCloseTags = (settings.autoCloseTags !== undefined) ? settings.autoCloseTags : true;
				settings.theme = (settings.theme !== undefined) ? settings.theme : 'default';
								
				this.codemirror.editor = CodeMirror.fromTextArea(this.codemirror.$textarea[0], settings);
				this.codemirror.editor.setOption('theme',settings.theme);
				this.codemirror.$textarea.next('.CodeMirror').hide();
				if(settings.fontSize !== undefined) this.codemirror.$textarea.next('.CodeMirror').css({fontSize:settings.fontSize});
			},
			toggle: function()
			{
